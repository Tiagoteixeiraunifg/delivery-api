package com.delivery.tiago.api.controller.v1.client;
...
public class ClientController {
	...
	/**
	 * Author: Tiago Teixeira Metodo verifica o perfil do usuário, caso não seja do
	 * tipo usuario é o admin com isso poderá ver todos os registros
	 * 
	 * @return
	 */
	@CrossOrigin(origins = "${front.baseurl}")
	...
	public ResponseEntity<Response<List<ClientesDTO>>> findAllClientes() {
		...
		if (userLoggd.getUserperfil().equals(UserPerfil.USUARIO)) {
			response.setData(clienteAssembler.toListClientesDTO(clienteRepository.findByUserId(userLoggd.getId())));
		} else {
			response.setData(clienteAssembler.toListClientesDTO(clienteRepository.findAll()));
		}
	...
	public ResponseEntity<Response<ClientesDTO>> saveCliente(@Valid @RequestBody ClientesDTO dto,
